...
import {loadRoster,loadAthletes} from '../lib/load.ts';
...
test('soft tennis rows with no committee names now show the five Chinese names',async()=>{
  const [data,r] = await Promise.all([daily(),loadAthletes()]);
  ...
  for (const row of rows) {
    ...
    assert.equal(shown.fromMaster,true);
...
test('regression: the outdated squad list never replaces the daily line-up (林蝶 vs 羅蘋)',async()=>{
  ...
  assert.equal(shown.fromMaster,false);
  ...
  const squad = (await roster()).disciplines.find(d=>d.disciplineCode==='BKB')!;
  assert.ok(squad.athletesZh.includes('羅 蘋'));
  assert.ok(!squad.athletesZh.includes('林蝶'));
});

test('an unverified name stays in English beside the verified ones',async()=>{
  ...
  assert.deepEqual(displayNames(row,master),{names:['余凱文','SOMEONE Unknown'],fromMaster:true});
});

test('a missing or broken master leaves every name in English',async()=>{
  assert.equal(await loadRoster('data/reference/does-not-exist.json'),null);
  const empty = parseAthleteMaster(null);
  const row = {disciplineCode:'TST',athletes:[],athletesEn:['YU Kai-wen']} as unknown as Row;
  assert.deepEqual(displayNames(row,empty),{names:['YU Kai-wen'],fromMaster:true});
  assert.deepEqual(parseAthleteMaster('not json').byEnglish.size,0);
});
