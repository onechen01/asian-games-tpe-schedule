...
export default function ScheduleExplorer({date,sports,initialFilters,jumpRequested,items,baseSummary,
  ...
  const finishFiltering=()=>{
    filterPanel.current?.removeAttribute('open');
  ...
  return <>
    <section className="schedule-tools" aria-label="賽程篩選">
        <details className="filter-panel" ref={filterPanel}>
          <summary ref={filterSummary}>篩選賽程{count>0&&<span>（已選 {count}）</span>}</summary>
          <div className="filter-panel-body">
            ...
            <button className="finish-filters" type="button" onClick={finishFiltering}>完成篩選</button>
          </div>
        </details>
      {active&&<div className="active-filters" aria-label="已套用的篩選條件">
        ...
        <button className="clear-filters" type="button" onClick={clear}>清除所有篩選</button>
      </div>}
    </section>
    <div className="schedule-locator" aria-label="賽程定位">
