...
export async function get(path: string, options: {conditional?:boolean} = {}): Promise<{data:unknown; meta:Meta}> {
  ...
  const disc = '[A-Z0-9]{3}';
  const pattern = new RegExp(`^(config|ALL/disc/list|ALL/schedule/matrix|${disc}/disc/data|${disc}/schedule/daily/\\d{4}-\\d{2}-\\d{2}|${disc}/(?:schedule/(?:unit|event)|results)/[A-Za-z0-9.\\-]+)$`);
