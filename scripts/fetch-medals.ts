...
// The outcome is returned rather than handed to process.exit(), which on Windows aborts while the
// standings connection is still open ("!(handle->flags & UV_HANDLE_CLOSING)") and made a successful
// unchanged-counts run report a non-zero status to the caller. Setting exitCode lets the event loop
// drain first, so update-results.ts logs what actually happened.
async function main():Promise<number> {
  ...
  if (same) { console.log('獎牌數未變動'); return 0; }
