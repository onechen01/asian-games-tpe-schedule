...
const entries = extractTpeEntries(data, meta.checked_at, ({ nameOnlyMissing, structuralMalformed })=>{
  console.log(`nameOnlyMissing=${nameOnlyMissing.length} structuralMalformed=${structuralMalformed.length}`);
  for (const bad of structuralMalformed) {
    console.error(`structural malformed index=${bad.index} Disc=${bad.disciplineCode ?? '無'}`
      + ` Reg=${bad.reg ?? '無'} Name=${bad.name ?? '無'} missing fields=${bad.missing.join(',')}`);
  }
  if (nameOnlyMissing.length) console.error(`略過 ${nameOnlyMissing.length} 筆缺 Name 報名資料；不代表未參賽。`);
});
